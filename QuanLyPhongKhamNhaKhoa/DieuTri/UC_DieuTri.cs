...
namespace QuanLyPhongKhamNhaKhoa.User_Control
{
    ...
    public partial class UC_DieuTri : UserControl
    {
        ...
        private void capNhatTrangThai()
        {
            ...
            {
                string appointmentID = comboBoxLichHen.SelectedValue.ToString();
                appointmentDao.updateStatusAppointment(appointmentID, "COMPLETED");
            }catch(Exception ex)
            {
        ...
        private void pbSearch_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtTreatmentID.Text.ToString() != "")
                {
                    SqlCommand command = new SqlCommand("SELECT * FROM Treatment WHERE treatmentID LIKE @treatmentID AND userID = @userID");
        ...
        private void btnXuatHoaDon_Click(object sender, EventArgs e)
        {
            ...
            report.reportpath = "E:\\SPKT\\NAM3_HK2\\WinForm\\Lù\\FinalProject\\QuanLyPhongKhamNhaKhoa\\Report_File\\ReportHoaDon.rdlc";
            report.dataset = "DataSetHoaDon";

            SqlCommand command = new SqlCommand();
            command.Connection = mydb.getConnection;
            command.CommandType = CommandType.StoredProcedure;

            command.CommandText = "GetPatientInfoAndServicesWithCost";

            command.Parameters.AddWithValue("@treatmentID", txtTreatmentID.Text.Trim());
        ...
        private void bbtnRefesh_Click(object sender, EventArgs e)
        {
            LoadAppointments();
        }
    }
